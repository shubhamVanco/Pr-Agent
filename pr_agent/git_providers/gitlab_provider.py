            ...
            try:
                ...
                if target_file.patch:
                    try:
                        ...
                        if added_lines and relevant_lines_start not in added_lines:
                            inside_range = [l for l in added_lines if relevant_lines_start <= l <= relevant_lines_end]
                            if inside_range:
                                relevant_lines_start = inside_range[0]
                            else:
                                relevant_lines_start = min(added_lines, key=lambda l: (abs(l - relevant_lines_start), l))
                            relevant_lines_end = max(relevant_lines_start, relevant_lines_end)
                    except Exception as e:
                        get_logger().warning(f"Could not snap suggestion to added line: {e}")

                range = max(0, relevant_lines_end - relevant_lines_start)
